<?php
    ...
    if(!isset($_SESSION["id"]) or !isset($_SESSION["classe"])  or !isset($_SESSION["UE"]))
    {
    ...
    $pages=scandir('Enseignant/');
   
   //on verifie si l'enseignant est le responsable de la discipline dans cette classe
   $reket="SELECT responsable from discipline_classe where code='$ue'";
   $resul=mysqli_query($con,$reket) or die("impossible d'exécuter la requet");
   $id_resp=mysqli_fetch_assoc($resul);

   if(mysqli_num_rows($resul)>0 AND $id_resp["responsable"]==$id)
   {
...
<?php
require_once(__DIR__ .'/Enseignant/'.$menu);
require_once(__DIR__ .'/Enseignant/barre_id_2.php');
require_once(__DIR__ .'/Enseignant/'.$page.'.php');
